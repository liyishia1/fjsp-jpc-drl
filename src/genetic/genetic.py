...
def elitistSelection(population, parameters):
    ...
    makespan_batch = []
    for i in population:
        makespan = timeTaken(i, parameters)
        makespan_batch.append(makespan)
    oldmakespan = copy.deepcopy(makespan_batch)
    zip_a_b = zip(makespan_batch, population)
    sorted_zip = sorted(zip_a_b, key=lambda x: x[0])
    sorted_a, sortedPop = zip(*sorted_zip)
    return list(sortedPop[:keptPopSize]), oldmakespan


def tournamentSelection(population, parameters, makespan):
    b = 2
    ...
    for i in range(b):
        ...
        selectmakespan.append(makespan[randomIndividual])
    minindex = selectmakespan.index(min(selectmakespan))
    return selectedIndividuals[minindex]


def selection(population, parameters):
    newPop, makespan = elitistSelection(population, parameters)
    while len(newPop) < len(population):
        newPop.append(tournamentSelection(population, parameters, makespan))

    return newPop, makespan
...
def crossoverOS(p1, p2, parameters):
    if random.choice([True, False]):
        counter = 0
        while True:
            newos = precedenceOperationCrossover(p1, p2, parameters)
            nes = 0
            for i in newos:
                istrue = jpc_check(i, parameters['jpc'])
                if istrue == False:
                    nes += 1
            if nes == 0:
                return newos
                ...
                counter += 1
        # print('poc-500 can not find jpc os')
        # return (p2, p1)
    else:
        counter = 0
        while True:
            newos = jobBasedCrossover(p1, p2, parameters)
            nes = 0
            for i in newos:
                istrue = jpc_check(i, parameters['jpc'])
                if istrue == False:
            ...
            if nes == 0:
                ...
                counter += 1
        # print('jbc-500 can not find jpc os')
        # return (p2, p1)
...
def mutationOS(p, parameters):
    if random.choice([True, False]):
        counter = 0
        while True:
            newos = swappingMutation(p)
            istrue = jpc_check(newos, parameters['jpc'])
            if istrue == True:
                return newos
            else:
                counter += 1
