from utils.common_utils import nonzero_averaging
...
class DualAttentionNetwork(nn.Module):
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, ope_pre_adj_batch=None, ope_sub_adj_batch=None, ope_ma_adj_batch=None):
        """
        ...
        for layer in range(self.num_dan_layers):
            ...
            # fea_j = self.op_attention_blocks[layer](fea_j, op_mask, ope_pre_adj_batch, ope_sub_adj_batch)
...
class DANIEL(nn.Module):
    def __init__(self, config):
        ...
        self.critic = Critic(config.num_mlp_layers_critic, 2 * self.embedding_output_dim, config.hidden_dim_critic,
                             1).to(device)
        # self.actor = Actor(config.num_mlp_layers_actor, 4 * self.embedding_output_dim,
        #                    config.hidden_dim_actor, 1).to(device)
        # self.critic = Critic(config.num_mlp_layers_critic, 2 * self.embedding_output_dim, config.hidden_dim_critic,
        #                      1).to(device)

    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, dynamic_pair_mask, fea_pairs, ope_pre_adj_batch=None, ope_sub_adj_batch=None, ope_ma_adj_batch=None):
        """
        ...
        #                                                               comp_idx, ope_pre_adj_batch, ope_sub_adj_batch, ope_ma_adj_batch)
        fea_j, fea_m, fea_j_global, fea_m_global = self.feature_exact(fea_j, op_mask, candidate, fea_m, mch_mask,
                                                                      comp_idx)
        sz_b, M, _, J = comp_idx.size()
        d = fea_j.size(-1)
