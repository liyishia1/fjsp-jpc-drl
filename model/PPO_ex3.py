from model.main_model_jpc import *
from utils.common_utils import eval_actions
import torch.nn as nn
import torch
from copy import deepcopy
from Params import train_configs, dan_configs
import numpy as np
...
class Memory:
    ...
    def push(self, state):
        ...
        self.dynamic_pair_mask_seq.append(state.new_dynamic_pair_mask_tensor)
...
class PPO:
    def __init__(self, train_configs, dan_configs):
        """
        ...
        self.gae_lambda = train_configs.gae_lambda
        self.eps_clip = train_configs.eps_clip
        self.k_epochs = train_configs.K_epochs
        self.tau = train_configs.tau

        self.ploss_coef = train_configs.A_coeff
        self.vloss_coef = train_configs.vf_coeff
        self.entloss_coef = train_configs.entropy_coeff
        self.minibatch_size = train_configs.minibatch_size

        self.policy = DANIEL(dan_configs)
    ...
    def update(self, memory):
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(int(num_batch)):
                ...
                loss1 = self.vloss_coef * v_loss
                loss2 = self.ploss_coef * p_loss
                loss3 = self.entloss_coef * ent_loss
                loss = loss1 + loss2 + loss3

                self.optimizer.zero_grad()
                loss_epochs += loss.mean().detach()
                p_loss_epochs += loss2.mean().detach()
                v_loss_epochs += loss1.mean().detach()
                e_loss_epochs += loss3.mean().detach()
                loss.mean().backward()
                self.optimizer.step()
        # soft update
        ...
        return loss_epochs.item() / self.k_epochs, p_loss_epochs.item() / self.k_epochs, v_loss_epochs.item() / self.k_epochs, e_loss_epochs.item() / self.k_epochs


def PPO_initialize():
    ppo = PPO(train_configs, dan_configs)
    return ppo
